...
green for Low). The twin is asset-agnostic: it is driven by whatever risk /
RUL the pipeline produced for the machine the user selects, so it works for a
motor, pump, or any rotating asset.
"""
...
function showError(msg){
  ...
  e.innerHTML = '<b>3D twin could not load three.js from CDN.</b><br>'+msg+
    '<br><br>The rest of the app is unaffected; check the browser network access to jsdelivr.';
}
...
// Drive-end BEARING — this is the health indicator.
...
function applyState(st){
  ...
  bearingMat.color.set(col);
  bearingMat.emissive.set(col);
...
window.__twinApply = applyState;   // allow future updates

const clock = new THREE.Clock();
function animate(){
  requestAnimationFrame(animate);
  const t = clock.getElapsedTime();
  shaft.rotation.x += 0.04;          // shaft spin
  bearing.rotation.z += 0.04;
  if (current.blink && current.speed>0){
    ...
    bearingMat.emissiveIntensity = 0.25;
    glowMat.opacity = 0.0;
  }
  controls.update();
...
def build_twin_html(
    ...
    """Return a self-contained HTML/JS 3D twin for embedding via st.components.v1.html."""
    ...
    return (
        _TEMPLATE.replace("__DATA__", json.dumps(payload))
