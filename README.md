...
- **Real live sources** — *Live Connect* adds **MQTT** and **OPC-UA** sources (alongside the simulator and CSV polling). MQTT expects JSON sensor payloads; OPC-UA reads a set of node IDs each poll. Requires `paho-mqtt` / `asyncua` (both in `requirements.txt`). **Demo path:** keep **Simulator** and press Start — no broker and no internet. MQTT `connect()` is async and localhost is probed in ~1s so a missing Mosquitto cannot freeze the Streamlit tab.
