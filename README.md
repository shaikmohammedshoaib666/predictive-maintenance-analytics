...
- **Real live sources** — *Live Connect* is the **ingestion bloodstream** (SIH26054): Simulator (HIL, aviation JSON when the Aviation pack is on) or **MQTT** (Mosquitto broker + Paho). Payloads are mapped to the same canonical row as CSV (CHT/EGT/oil/fuel; `uav_id`; bar→psi). Physics + Isolation Forest + RUL score on **Refresh live** / Start. UI auto-refresh defaults **off** (`LIVE_REFRESH_SECONDS=0`) so Render free can finish first paint; do not use a 3s fragment. **Use flight log in pipeline** dumps the stored sortie into Map → Detect → Charts. ATA-100 chapters sit on physics faults. MQTT user/pass from `MQTT_USER` / `MQTT_PASS`. **Demo path:** Aviation pack → Live Connect → Simulator → Start live → **Refresh live**. Localhost MQTT on Render is the Render VM. Optional: `python hil_mqtt_publisher.py`.
- **CAD Twin (Autodesk APS)** — *CAD Twin* page + dashboard CAD tile. Availability-gated at **runtime**: set `APS_CLIENT_ID` + `APS_CLIENT_SECRET` on Render after deploy and reload — no code change. With credentials, choose a CAD file on *CAD Twin*, **Translate to SVF / get URN** (that URN is in **your** OSS bucket). `viewer.autodesk.com` is Autodesk’s public site and does **not** put the file in your bucket. Streamlit `maxUploadSize` is **300 MB** so a ~207 MB STEP can upload; if Render still blocks the POST, zip the STEP or export a lighter STEP from Fusion. Last successful URN is saved per industry pack in `data/cad_urns.json` on the server so Aviation still loads after you close the browser (while this service is up). `APS_MODEL_URN` overrides that file and is what survives a Render **redeploy**. The embed is **GuiViewer3D** (full toolbar: Home / Fit / Pan / Zoom / Orbit / First Person, Measure, Section, Explode, View cube, Model browser, Properties, Settings, Views, Markup when the library ships it) — not a headless Viewer3D. A pasted `viewer.autodesk.com` URL is scanned for a `dXJu…` URN, but that public-viewer object will not load with your 2-legged token; translate the STEP on this page instead. Until APS is configured the 3D twin is the credential-free mesh.
- **CAD region map persist** — click-to-assign dbIds still write `data/cad_region_map.json` (gitignored). CAD Twin can **export / import** that JSON. Optional `CAD_MAP_JSON` secret seeds a map on boot (honest Render path — the disk still resets on redeploy). Browser localStorage is a backup, not a database.
- **Live last-seen** — Live Connect MQTT host/port/topic also read `MQTT_BROKER` / `MQTT_PORT` / `MQTT_TOPIC`. Per-asset last-seen + stale flag. Localhost on Render is the Render VM, not hangar telemetry.
- **Fleet / assets** — Aviation Dashboard leads with every tail, mission %, MissionAdvisory, go/no-go, plus a thin work-order stub (`WO-UAV-03-OIL`). Isolation Forest stays the default ML; physics stays Layer 1.
- **Health-event JSON** — download a digital-thread record (sensors, physics, IF/RUL, advisory, twin region) from Anomaly or Dashboard after Detect. Export-only; no second Render service.
