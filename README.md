...
- **CAD Twin (Autodesk APS)** — *CAD Twin* page + dashboard CAD tile. Availability-gated at **runtime**: set `APS_CLIENT_ID` + `APS_CLIENT_SECRET` on Render after deploy and reload — no code change. With credentials, choose a CAD file on *CAD Twin*, **Translate to SVF / get URN** (that URN is in **your** OSS bucket). Last successful URN is saved per industry pack in `data/cad_urns.json` on the server so Aviation still loads after you close the browser (while this service is up). `APS_MODEL_URN` overrides that file and is what survives a Render **redeploy**. The embed is **GuiViewer3D** (full toolbar: Home / Fit / Pan / Zoom / Orbit / First Person, Measure, Section, Explode, View cube, Model browser, Properties, Settings, Views, Markup when the library ships it) — not a headless Viewer3D. A pasted `viewer.autodesk.com` URL is scanned for a `dXJu…` URN, but that public-viewer object will not load with your 2-legged token; translate the STEP on this page instead. Until APS is configured the 3D twin is the credential-free mesh.
