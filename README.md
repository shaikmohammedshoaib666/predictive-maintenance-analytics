...
- **Real live sources** — *Live Connect* is the **ingestion bloodstream** (SIH26054): Simulator (HIL, aviation JSON when the Aviation pack is on) or **MQTT** (Mosquitto broker + Paho). Payloads are mapped to the same canonical row as CSV (CHT/EGT/oil/fuel; `uav_id`; bar→psi). Physics + Isolation Forest + RUL score on **Refresh live** / Start (no auto-reload — that looped Streamlit until HTTP 429). **Use flight log in pipeline** dumps the stored sortie into Map → Detect → Charts. ATA-100 chapters sit on physics faults. MQTT user/pass from `MQTT_USER` / `MQTT_PASS`. **Demo path:** Simulator — no broker. Localhost on Render is the Render VM. Optional: `python hil_mqtt_publisher.py`.
